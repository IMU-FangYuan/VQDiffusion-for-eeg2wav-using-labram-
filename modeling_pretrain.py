...
class NeuralTransformerForMaskedEEGModeling(nn.Module):
    ...
    def forward_features(self, x, input_chans, bool_masked_pos=None):
        ...
        #print(x.shape,pos_embed_used.shape,self.pos_embed.shape,"posembedshape")
    ...
    def forward(self, x, input_chans=None, bool_masked_pos=None, return_all_tokens=False, return_patch_tokens=False, return_all_patch_tokens=False):
        ...
        x = self.forward_features(x, input_chans=input_chans, bool_masked_pos=None)
...
class NeuralTransformerForMEM(nn.Module):
    ...
    def forward(self, x, input_chans=None, bool_masked_pos=None):
        ...
        #print(x_masked.shape,x_masked_no_cls.shape,x_masked_no_cls.shape,"x masked")
        #x_rec = self.lm_head(x_masked_no_cls)

        #symetric
        #x_masked_sym = self.student(x, input_chans, ~bool_masked_pos, return_all_patch_tokens=True)
        #x_masked_no_cls_sym = x_masked_sym[:, 1:]
        #x_rec_sym = self.lm_head(x_masked_no_cls_sym)
...
class EmbeddingModel(nn.Module):
    def __init__(self,):
        super().__init__()
        self.lambra = labram_base_patch200_1600_8k_vocab(  drop_path_rate=0.1,
    ...
    def forward(self, x, input_chans=None, bool_masked_pos=None):
        x_masked = self.lambra(x, input_chans, bool_masked_pos)
        t = x_masked[:, 1:, :]
        
        
        
        out = self.ln(t.mean(1))
        
       
        return out #, x_rec_sym
...
if __name__ == '__main__':
    ...
    #print(statedict['logit_scale'].shape,statedict['logit_scale'])
    #for key in statedict.keys():
    #    print(key)
    model.lambra.load_state_dict(checkpoint["model"])
    x = torch.randn(1,62,4,200)
    
    out = model(x,input_chans = 63,bool_masked_pos = None)
    print(out.shape)